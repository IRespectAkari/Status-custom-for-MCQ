...
function addAlchemyAlart() {
  ...
  switch(getData("依頼内容")){
    case "鍛冶":
      if (isMax()) {
        if (howMenyItems("上質砥石") > 0) {
          createNotionDiv("鍛冶に「上質砥石」が使用される可能性があります");
        } else if (howMenyItems("砥石") > 0) {
          createNotionDiv("鍛冶に「砥石」が使用される可能性があります");
        }
        return;
      }
      if (howMenyItems("賢者の石") > 0) {
        createNotionDiv("鍛冶に「賢者の石」が使用される可能性があります");
      } else if (howMenyItems("錬成石") > 0) {
        createNotionDiv("鍛冶に「錬成石」が使用される可能性があります");
      } else if (howMenyItems("上質砥石") > 0) {
        createNotionDiv("鍛冶に「上質砥石」が使用される可能性があります");
      } else if (howMenyItems("砥石") > 0) {
        createNotionDiv("鍛冶に「砥石」が使用される可能性があります");
      }
...
function addBlacksmithAlart() {
  ...
  // 砥石・強化石系の使用
  // for (let e of isMax() ? blacksmithListMax : blacksmithListUnmax) {
  for (let e of blacksmithListMax) {
    if (isHaving(e)) {
