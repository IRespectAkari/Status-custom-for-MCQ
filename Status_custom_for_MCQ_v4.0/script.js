...
////////////////////////////////////////////////////////////////////
function addAlchemyProbability() {
  if (get("AlchemyProbability") !== "有効") {
    return;
  }

  // 錬金術の対象になるアイテムがチェックボックスと一緒にならぶ
  // それを取得する
  const alchemyObject = getDocument().querySelector();
  for (let e of alchemyObject) {
    const txt = [
      `e : ${e}`,
      `label : ${e.innerText}`,
      `chara : ${getValue("ヒーロー")}`,
      ``,
      `probability : ${alchemyProbabilityList[e]}`,
    ].join("\n");
    console.log(txt);
  }
}
